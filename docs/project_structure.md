...
| `package.json` | 依赖：three 0.184、page-flip 2.0.7、react 19、vite 8。脚本 dev/build/asset:audit/commentary:validate/visual:smoke/narrative:smoke/**voice:smoke（S18）**/preview |
...
| `src/narrativeBeats.js` | ~145 | **Sprint 15 primary-event 纯数据层**：`buildNarrativeBeatIndex({spreadCount,sources,pageToSpread})→Map<spread,event>`、`resolveNarrativeBeat`、`narrativeEventText`。支持可选 `commentary.json` `beat` / `focus` metadata（type/label/prompt/emphasis/cue/target），无 metadata 时按 runway → commentary → lookCard → standee → read 确定 fallback，保证每个 spread 恰好一个 primary event；无 DOM / WebGL / `this.standees` 依赖，可被 `npm run narrative:smoke` headless 断言 |
| `src/voice.js` | ~80 | **S18 语音 URL 解析纯逻辑**：`voiceUrlFor(packId, voicePath)` / `resolveVoiceUrl(index, …)`（可注入索引便于断言）/ `buildVoiceUrlIndex` / `normalizeVoicePath`。**build-time glob `import.meta.glob('../assets/image-packs/*/voice/*')`（今天为空 → 每次解析 null），镜像页图解析范式，缺文件 → null，绝不运行期手拼路径 fetch**。无 DOM/场景/AudioContext 依赖、可 headless 断言。`magazineScene.js` 顶部 `decorateCommentaryVoice` 用它把 `intro`/`item`/`runwayIntro` 一次性解析为 `voiceUrl`（浅拷贝行、不改共享 eager-glob JSON） |
...
| `src/styles.css` | ~1295 | 全部 UI 样式：HUD（`.hud-read`/`.hud-tour`/`.hud-locale`/`.hud-share`/**`.hud-card`=CARD-1** pill、`.hud-masthead-actions` 报头动作行、`.hud-character` 报头跟随块、**`.hud-beat`=Sprint 15 primary-event chip**）、loader、cursor、鑑賞 overlay、触控/键盘提示（`pointer:coarse` **或** `max-width:640px` 切换；窄屏 `.hud-touch` 上抬 = A1，`.hud-feature` 装饰竖排隐藏以避开 controls/standee；`.hud-beat-prompt` 在窄屏隐藏）、响应式。**G1：`.gallery-toc`/`.gallery-thumb` 等。I1：`@media(max-width:640px)` 收口底部 HUD 列（列高 159px→~124px）。CARD-1：`.look-card`（z-index:8 寄生 overlay、scrim+paper sheet、纸色 #f6efe0/墨 rgba(30,24,18,*)/Shippori Mincho 纸艺语汇）/`.look-card-sheet`(375px 全屏可滚 sheet / 1280px 居中 max 560px)/`.look-card-item`(`.is-active` 高亮)/`.look-card-backlink`/`.look-card-share`，reduced-motion 媒体块补 `.look-card` 无过渡** |
| `src/magazineScene.js` | ~5550 | **核心**：three.js 场景 + 全部交互逻辑（下表为行号地图）。TECH-1（Iter 3）后顶部 import 增加 `./render-config.js`；Iter 4 RACE-B-小批 +21 行；Sprint 13a 新增 background texture GPU warmup queue。**Sprint 14**：新增本地 `MATERIAL_VISUALS` + `onBeforeCompile` paper/standee material layers、turn leaf cue overlay、GrainShader post grade；不改 DOM 鑑賞、raw standee TextureLoader、camera framing 或 toneMapping。**Sprint 15**：接入 `narrativeBeats.js` primary-event resolver、`.hud-beat` 小标签、主/次按钮态、per-session discovery cues、lookCard/tour/show/gallery guard 补强；不持久化临时 narrative 状态 |
...
| 1179-1450 | `spreadLeftPage`/`spreadRightPage`/`pageToSpread`/`spreadCommentaryIndex`/`narrativeBeatIndex`/`resolvedPrimaryEvent`/`applySpread` | 正向 spread→page + **反向 `pageToSpread()` 单一真相源（D1，buildStandee 与 D2 退鑑賞回写共用，越界返 null）** + **CARD-1 `spreadCommentaryIndex()`：lazily memoize `buildSpreadCommentaryIndex(STANDEE_SOURCES, pageToSpread)`（一处建、入口 guard + 卡数据两处用、绝不碰 this.standees）** + **Sprint 15 `narrativeBeatIndex()`：同样从 module-level STANDEE_SOURCES + pageToSpread 建 primary-event Map，绝不碰 runtime this.standees** + open 态材质落定 |
...
| 2721-2755 | `maybeShowStandeeHint`/`maybeShowStandeeGuide`/`showRunwayHint` | 起立前「点她」一次性提示 / 起立后「服にふれると、コーデ解説」/ runway 提示，均走 Sprint 15 `tryDiscoveryCue()` per-session guard；热点 bloom 脉冲仍仅 `!reducedMotion` 时设 |
| 2761-2869 | `startShow`/`endShow`/`finishShow`/`applyShowDim` | 暗场走秀状态机；Sprint 15 入口 guard：gallery/lookCard/turn 活跃时 return，tour 活跃时 close 后进入 show。**TECH-1：applyShowDim 中 `grainPass.uniforms.uVignette.value = RENDER.grain.vignette + RENDER.grain.runwayDelta * eased`，与 GrainShader 默认共用同一 `RENDER.grain.vignette`，物理上锁死 Iter 2 同步规则** |
...
| 3506/3653/3678 | `createHud`/`syncHud`/`syncTourButton` | HUD 构建（含 `.hud-read` 鑑賞、`.hud-tour` 解说巡回、`.hud-locale` 语言切换 = C7、`.hud-share` 分享 + `.hud-masthead-actions` 行 = E5、`.hud-character` 报头跟随块 = C8、**`.hud-beat` primary-event chip = Sprint 15**）/ 页码同步 / 巡回 pill 逐帧显隐 |
| 3687-3839 | `syncLocaleButton`/`currentSpreadCharacter`/`syncMasthead`/`syncCardEntry`/`currentNarrativeEvent`/`tryDiscoveryCue`/`syncNarrativeBeat` | C7 语言按钮态同步 / C8 当前跨页角色查询 + 报头跟随 / CARD-1 `.hud-card` guard / **Sprint 15 primary-event HUD sync**（小标签、按钮主次态、per-session discovery cue；reduced-motion 不加 `.is-discovery` animated emphasis） |
...
| 4115-4297 | `openLookCard`/`renderLookCard`/`escapeHtml`/`closeLookCard`/`copyLookCardLink` | **CARD-1 看卡**：`openLookCard(spread,item)`（寄生 overlay 挂载、与鑑賞互斥先 closeGallery、guard 走索引、reduced-motion 守、backlink 走 `landOnSpread(persist:false)`、卡/item 不 persist；Sprint 15 入口 guard：show/turn return、tour close、peel clear）/ `renderLookCard`（按 locale 渲染字段，setLocale 时若卡开则重渲=per-locale 红线）/ `closeLookCard`（reduced-motion 同步移除）/ `copyLookCardLink`（卡内分享，带 item）。键盘：卡开时 Esc/Space 关卡、吞其它键（`handleKeyDown` 顶部） |
| 4050/4091 | `startAudio`/`playSound` + **S18 语音方法群** | WebAudio 总线（音频文件缺失时应静默降级）。**S18**：`startAudio` 同时建 `voiceGain`→`voiceAnalyser`→destination 并在首手势 resume ctx（单一解锁路径，不新增第二个）；`duckRoomTone` 压低/还原 room tone；`decodeVoiceBuffer`（缓存，smoke 可预置合成 buffer 跳过 fetch）；`playVoice`/`stopVoice`（单一进出口，新一条停上一条、同热点 280ms 去抖、缺 voiceUrl 退化为纯字幕）；`resolveMouthCells`/`onVoiceEnded`/`releaseMouth`；`clearCaptionFade`/`scheduleCaptionFade`（字幕 900ms 淡出）；`updateLipSync`（animate 每帧调，RMS 驱动 mouth-open/closed，仅当有合法 `mouth` 映射且非 reduced-motion，否则 no-op）。dispose 停活跃语音 + 断 voice 节点 + 清 captionFadeTimer + 清 voiceUrlCache。模块顶部 `decorateCommentaryVoice`/`resolveLineVoice` 用 `src/voice.js` 把每条 line 解析出 `voiceUrl`（TDZ：必须在 PAGE_PACKS IIFE 之前声明） |
...
| 4579-4880 | `galleryEntries`/`galleryStartIndex`/`toggleGallery`/`galleryFlip`/`closeGallery`/`applyGalleryLanding` | **鑑賞 DOM 翻页书**（StPageFlip，loadFromHTML 真实 img 页）。`toggleGallery` 内建 `.gallery-toc` + `⊞` 切换；进入 gallery 清 stale turn 四件套、关闭 lookCard/tour、清 peel；进入/退出显式 `syncNarrativeBeat(true)`，因为 gallery 期间 animate early-return。D2：`closeGallery` 退出读当前页→`applyGalleryLanding`，cover/back/colophon 专路，page 类走 `pageToSpread`→`landOnSpread`，回写前 `foldStandees(true)` |
| 4909/4926 | `handleResize` / `animate` | resize / 主循环（animate 在 `gallery` early-return 后每帧 drain 1 个 queued texture，并调 `syncTourButton`+`syncMasthead`+`syncNarrativeBeat`） |
...
| 5078 | `dispose` | 清理（含 queued texture warmup 清空、Sprint 15 discovery timers 清空、`window.__magazineScene` 注销） |
