...
## 语音前端 / 口型同步（Sprint 18 新增）

- **build-time glob 装饰器必须声明在调用它的 module-level IIFE 之前**（S18-BUS-1 TDZ 同模式坑）：`PAGE_PACKS`（magazineScene.js 顶部 IIFE，模块求值即运行）调用 `decorateCommentaryVoice` 把每条 commentary line 解析出 `voiceUrl`。早期按「就近归类」把这个 helper 放到 `localeText` 旁（IIFE 之后）→ `ReferenceError: Cannot access 'decorateCommentaryVoice' before initialization`（`const` 不提升进可访问态、TDZ）。**Vite 构建期此 throw 会让整个 magazineScene 模块求值失败、scene 永不 `assetsReady`**，headless smoke 直接在 `waitForExpression(assetsReady)` 60s 超时，错误信息**不指向真因**（要靠 `Runtime.exceptionThrown` 捕获页内异常才看到）。正解：任何被 module-level 数据 IIFE 调用的 helper 必须声明在 IIFE 之前（本轮 `resolveLineVoice`/`decorateCommentaryVoice` 移到 `backCoverUrl` 之后、`packReaderFileUrls` glob 之前）。**模式归档**：smoke「卡在 assetsReady 超时」不一定是资源加载慢，先用 CDP `Runtime.enable` + 监听 `exceptionThrown`/console 看模块求值期有没有 throw。
- **headless Chrome 实时 AudioContext 时钟在推进但 AnalyserNode 只在 source 正播放的帧读到信号**（S18-VERIFY-1 同模式坑）：voice smoke 早期用 0.5s 合成 buffer，source-wait 循环（最长 ~1.9s）+ buffer 在采样窗口前就 `onended`→`onVoiceEnded` 置 `this.voice=null`→`updateLipSync` 返回 0、analyser 读纯静默（`getByteTimeDomainData` 全 128、rawByteSpread=0），误判「analyser 不工作」。**正解**：合成 buffer 拉长到 ≥4s 覆盖整个采样窗口 + 启 Chrome 加 `--autoplay-policy=no-user-gesture-required`。**模式归档**：任何「headless 验 analyser / 实时音频 RMS」的 smoke 必须保证被测 source 在采样期间仍在播放，且断言「过程中是否出现过 open」而非「采样结束时的瞬时 cell」——口型 open/closed 逐帧切换、末帧值是 racy 的（`writeExpressionUv` 写瞬时 mouth UV、不更新 `card.cell`，所以读 `mouth.lastOpen` 不读 `card.cell`）。
- **语音/字幕拆除靠单一 `stopVoice`+`hideCaption` 接到既有七状态拆字幕点**（S18-SUBTITLE-1 / RACE-B 同模式坑延续）：voice 是增强层，每一个今天就会拆字幕的状态转换都必须同步拆 voice。本轮接入点：beginTurn（经 foldStandees）/toggleGallery/openLookCard/startShow/finishShow/foldStandees/endTour/hideCommentary。`stopVoice` 幂等（无 session 也安全）、还原 room tone duck、释放口型；`scheduleCaptionFade` 在 tour 活跃时 no-op（tour 自己管字幕节奏）。**未来新增任何会拆字幕的 overlay/状态转换都要同步加 `stopVoice()`**，与 toggleGallery 既有 `if (this.tour) this.endTour()` 模板对称。
- **口型同步不可与眨眼循环打架**（S18-LIPSYNC-1 同模式坑）：`updateStandees` 的眨眼循环逐帧写表情 UV；口型同步活跃时也写 mouth UV。本轮在眨眼分支加 `this.activeMouthCard !== card` 守，让活跃口型卡的 UV 由 lip-sync 独占。当前素材无 mouth 格→`activeMouthCard` 恒 null→眨眼分支不受影响（零回归）。**未来出 mouth-cell 素材后**，口型与眨眼/表情 hint 仍走「lip-sync 活跃时独占该卡 UV」这条线，别让两个写 UV 的循环并发。
